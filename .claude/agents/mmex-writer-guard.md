...
description: Audits the MMEX writer (OFX export and direct SQL writer). Use proactively when files in src/finanzasmmex/writer/, MMEX deduplication logic, backup configuration, or anything touching .mmb files are modified. Blocks any path that could touch productive finanza.mmb before shadow-mode validation.
...
If `production_path_check.status` is `blocked`, the entire writer cannot merge. State this explicitly in `summary`.
